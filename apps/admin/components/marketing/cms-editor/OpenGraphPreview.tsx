  ...
  return (
    <div className="overflow-hidden rounded-lg border border-slate-200">
      ...
      {img ? (
        ...
        <div className="flex aspect-[1.91/1] items-center justify-center bg-slate-100 text-xs text-slate-400">Không có ảnh</div>
      )}
      <div className="p-2">
        <p className="truncate text-[10px] uppercase text-slate-400">{new URL(resolvePublicUrl(url)).hostname}</p>
        <p className="line-clamp-2 text-xs font-semibold text-slate-900">{title || 'Tiêu đề'}</p>
        <p className="line-clamp-2 text-[11px] text-slate-500">{description || 'Mô tả'}</p>
      </div>
    </div>
  );
}
...
export function OpenGraphPreview({
  ...
  return (
    <div className="space-y-2">
      <p className="text-[10px] font-medium uppercase text-slate-400">OpenGraph Preview</p>
      <div className="grid grid-cols-2 gap-2">
        ...
        <OgCard platform="X" title={title} description={description} image={image} url={url} accent="bg-slate-100 text-slate-800" />
      </div>
    </div>
  );
}
