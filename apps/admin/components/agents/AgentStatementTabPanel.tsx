...
export function AgentStatementTabPanel({
  ...
  const mainRequestsRef = useRef(new LatestRequestTracker<'main'>());
  const adjustmentRequestsRef = useRef(new LatestRequestTracker<'adjustments'>());
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!mainRequestsRef.current.isLatest(token)) return;
      setDashboard(dash);
      ...
      if (mainRequestsRef.current.isLatest(token)) {
        setLoading(false);
      }
    }
  }, [agentId, periodParams, searchQuery]);

  const loadAdjustments = useCallback(async (statementId?: string | null) => {
    const token = adjustmentRequestsRef.current.begin('adjustments');
    try {
      ...
      if (!adjustmentRequestsRef.current.isLatest(token)) return;
  ...
  useEffect(() => {
    mainRequestsRef.current.invalidateAll();
    adjustmentRequestsRef.current.invalidateAll();
    detailRequestsRef.current.invalidateAll();
    const next = presetDates('this_month');
  ...
  useEffect(() => {
    const timeout = window.setTimeout(() => setSearchQuery(search.trim()), 350);
  ...
  const closeDetail = useCallback(() => {
    detailRequestsRef.current.invalidate('detail');
    setDetailOpen(false);
    setDetail(null);
  }, []);
  ...
  return (
    <div className="space-y-6">
      ...
      {loading && <p className="text-sm text-slate-500">Đang tải dữ liệu sao kê…</p>}
      ...
      {canWrite && (
        <Card className="flex flex-wrap gap-2 p-4">
          ...
          <Link href={`/agents/${agentId}?tab=orders`} className="inline-flex">
      ...
      <Card className="p-4">
        <h3 className="font-semibold text-slate-900">Chu kỳ sao kê</h3>
              ...
              {statements.length === 0 ? (
                ...
                statements.map((s) => {
                  ...
                  return (
                    <tr
                      key={id}
                      className={cn('border-b border-slate-50', isSelected && 'bg-admin-50/40')}
                    >
                      <td className="py-2 pr-4 font-medium">{formatDisplayValue(s.periodLabel)}</td>
                      <td className="py-2 pr-4 text-xs text-slate-600">
                        {formatDisplayValue(s.periodStart).slice(0, 10)} →{' '}
                        {formatDisplayValue(s.periodEnd).slice(0, 10)}
                      </td>
                      <td className="py-2 pr-4 font-medium">{formatVnd(net)}</td>
                      <td className="py-2 pr-4">
                        <Badge
                          status={formatDisplayValue(s.status)}
                          tone={statusTone(formatDisplayValue(s.status))}
                        />
                      </td>
      ...
      {canWrite && (
        <Card className="space-y-3 p-4">
          <h3 className="font-semibold text-slate-900">Điều chỉnh thủ công</h3>
          {selectedStatement ? (
            <p className="text-sm text-slate-600">
              Gắn với kỳ <strong>{formatDisplayValue(selectedStatement.periodLabel)}</strong>
          ...
          <ul className="space-y-2 text-sm">
            {adjustments.map((a) => (
              <li key={String(a.id)} className="flex justify-between border-b border-slate-50 py-2">
                <span>{formatDisplayValue(a.reason)}</span>
        ...
        <table className="min-w-full text-sm">
          ...
          <tbody>
            {orders.length === 0 ? (
              ...
              orders.map((o) => (
                <tr key={String(o.orderId)} className="border-b border-slate-50">
                  <td className="px-3 py-2 font-mono text-xs">
                    <Link
                      href={`/orders/${String(o.orderId)}`}
                      className="text-admin-600 hover:underline"
                    >
                      {formatDisplayValue(o.orderCode)}
                    </Link>
                  </td>
                  <td className="px-3 py-2">{formatDisplayValue(o.partnerOrder)}</td>
                  <td className="px-3 py-2">{formatDisplayValue(o.product)}</td>
                  <td className="px-3 py-2">{formatVnd(formatDisplayValue(o.providerCost))}</td>
                  <td className="px-3 py-2">{formatVnd(formatDisplayValue(o.agentPrice))}</td>
                  <td className="px-3 py-2">{formatVnd(formatDisplayValue(o.profit))}</td>
                  <td className="px-3 py-2">{formatDisplayValue(o.status)}</td>
                </tr>
              ))
            )}
          </tbody>
      ...
      <DetailDrawer
        ...
        onClose={closeDetail}
        title={`Sao kê ${formatDisplayValue(detail?.periodLabel)}`}
      >
        {detail && (
          <>
            <div className="grid grid-cols-2 gap-2 text-sm">
              <div>
                ...
                <Badge
                  ...
                  tone={statusTone(formatDisplayValue(detail.status))}
                />
              </div>
              <div>
                <p className="text-slate-500">Thanh toán</p>
                <p>{formatDisplayValue(detail.paymentStatus)}</p>
              </div>
              <div>
              ...
              {detail.lockedAt ? (
                <div>
                  <p className="text-slate-500">Khóa lúc</p>
            ...
            {Array.isArray(detail.timeline) && (
              <div>
                ...
                <ul className="mt-2 space-y-2 border-l border-slate-200 pl-3 text-sm">
